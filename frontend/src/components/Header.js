...
const Header = ({ user, onLogout }) => {
  ...
  return (
    <header className="glass sticky top-0 z-50 border-b border-emerald-500/20">
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-r from-emerald-400 to-yellow-500 rounded-lg flex items-center justify-center pulse-glow">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-all duration-300 relative ${
                    isActive
                      ? 'nav-active'
                      : 'text-green-200 hover:text-emerald-400 hover:bg-emerald-800/30'
                  } ${item.special ? 'ai-notification' : ''}`}
                >
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden sm:flex items-center space-x-3">
              ...
              <div className="text-sm">
                ...
                <p className="text-emerald-400 crypto-font">{user.free_predictions} прогнозов</p>
              </div>
            </div>
            
            <button
              onClick={onLogout}
              className="p-2 text-green-300 hover:text-red-400 transition-colors duration-300"
              title="Выйти"
            >
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center space-x-2 px-3 py-2 rounded-xl whitespace-nowrap transition-all duration-300 relative ${
                  isActive
                    ? 'nav-active'
                    : 'text-green-200 hover:text-emerald-400 hover:bg-emerald-800/30'
                } ${item.special ? 'ai-notification' : ''}`}
